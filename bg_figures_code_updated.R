####  FIGURE 1 - ALPHA DIVERSITY LINE GRAPHS ###############
...
####  FIGURE 2 - PHYLUM RELATIVE ABUNDANCE ##############
...
####  FIGURE 3 - INTESTINAL BIOGEOGRAPHY ####
...
plot.pseq <- tax_glom(pseq.rarefied.sub, taxrank="Genus")
plot.pseq <- transform_sample_counts(plot.pseq, function(x) 100*x/sum(x))
plot.pseq <- prune_taxa((taxa_sums(plot.pseq)/nsamples(plot.pseq)) > 0.2, plot.pseq)
...
colnames(temp) <- as.data.frame(cbind(tax_table(plot.pseq)))$Genus
...
temp.tax <- subset(temp.tax, Genus != "Uncl_Firmicutes" & Genus !="Uncl_Clostridiales")
levels(temp.tax$Genus)[levels(temp.tax$Genus)=="Uncl_Lachnospiraceae"] <- "Lachnospiraceae spp."
levels(temp.tax$Genus)[levels(temp.tax$Genus)=="Uncl_Rhodobacteraceae"] <- "Rhodobacteraceae spp."
levels(temp.tax$Genus)[levels(temp.tax$Genus)=="Escherichia/Shigella"] <- "Escherichia"
levels(temp.tax$Genus)[levels(temp.tax$Genus)=="Uncl_Fusobacteriaceae"] <- "Fusobacteriaceae spp."

heatmap.data$rowname <- factor(as.character(heatmap.data$rowname))
levels(heatmap.data$rowname)[levels(heatmap.data$rowname)=="Uncl_Lachnospiraceae"] <- "Lachnospiraceae spp."
levels(heatmap.data$rowname)[levels(heatmap.data$rowname)=="Uncl_Rhodobacteraceae"] <- "Rhodobacteraceae spp."
levels(heatmap.data$rowname)[levels(heatmap.data$rowname)=="Escherichia/Shigella"] <- "Escherichia"
levels(heatmap.data$rowname)[levels(heatmap.data$rowname)=="Uncl_Fusobacteriaceae"] <- "Fusobacteriaceae spp."

temp.tax <- arrange(temp.tax, Phylum, Family, Genus)
temp.tax$Genus <- factor(temp.tax$Genus)
heatmap.data <- subset(heatmap.data, rowname %in% temp.tax$Genus)
temp.tax <- mutate(temp.tax, Genus = factor(Genus, Genus))

heatmap.data$rowname <- factor(as.character(heatmap.data$rowname), levels=levels(temp.tax$Genus))

# Assign colors for each phylum.
colornames <- c("#CC0000", "#CC0000", # 2x Actinobacteria
                "magenta3", "magenta3", "magenta3",  # 3x Bacteroidetes
                "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", #16x Firmicutes
                "darkgreen", "darkgreen", # 2x Fusobacteria
                "#0000FF", "#0000FF", "#0000FF", "#0000FF", "#0000FF", "#0000FF", "#0000FF", "#0000FF") # 8x Proteobacteria
names(colornames) <- levels(heatmap.data$rowname)

# Extract summary data from mixed effects logistic regressions. #
logit.data <- coeff.logits.mix.eff[["Genus"]]

# Obtain the same taxa that were used in the heat map, and sort by phylum, then alphabetically by family. 
logit.data$Genus <- as.factor(logit.data$Genus)
levels(logit.data$Genus)[levels(logit.data$Genus)=="Escherichia/Shigella"] <- "Escherichia"
levels(logit.data$Genus)[levels(logit.data$Genus)=="Uncl_Lachnospiraceae"] <- "Lachnospiraceae spp."
levels(logit.data$Genus)[levels(logit.data$Genus)=="Uncl_Rhodobacteraceae"] <- "Rhodobacteraceae spp."
levels(logit.data$Genus)[levels(logit.data$Genus)=="Uncl_Fusobacteriaceae"] <- "Fusobacteriaceae spp."

logit.data <- subset(logit.data, Genus %in% temp.tax$Genus)
logit.data$Genus <- factor(logit.data$Genus, levels=levels(temp.tax$Genus))
logit.data <- arrange(logit.data, Genus)

# Replace family names with numbers (to avoid taking up too much space on the plot).
levels(logit.data$Genus) <- c(1:29)
logit.data$Genus <- as.factor(logit.data$Genus)
...
plot.data$Site <- factor(plot.data$Site, levels=c("duodenum", "jejunum", "ileum", "caecum", "ascending.colon", "descending.colon", "feces"))

# Clean some names.
levels(heatmap.data$rowname)[levels(heatmap.data$rowname)=="Clostridium_sensu_stricto"] <- "Clostridium"

heatmap.data$rowname <- factor(heatmap.data$rowname)
logit.data$Genus <- factor(logit.data$Genus)
...
Fig3.logits <- ggplot(logit.data, aes(x=Genus, y=coeff)) + 
  geom_point(color=colornames) +
  theme_bw() +
  geom_errorbar(aes(ymin=logit.data$`2.5`, ymax=logit.data$`97.5`, x=Genus), position=position_dodge(0.9), width=0.2, color=rev(colornames)) +
  geom_hline(yintercept=0) +
  labs(x=element_blank(), y="Coefficient") +
  ylim(-3,3) +
  coord_flip() +
  scale_x_discrete(limits = rev(levels(logit.data$Genus))) +
  theme(
...
####  FIGURE 4 - dbRDA STRUCTURING EFFECTS ##############
...
dbrda.plot.a <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  ...
  geom_path(data=ind.hulls[["df_hull"]], aes(x=2.45*x, y=3.2*y, linetype=Group), size=0.5, color="grey") +
  ...
  geom_label(aes(x=1.90187, y=0.4289), color="black", label="LI") +
  geom_label(aes(x=-1.96745, y=-0.4437), color="black", label="SI") +
  new_scale("linetype") +
  ...
  scale_y_continuous(breaks=c(-7.5,-5,-2.5,0,2.5,5), limits=c(-8,6.3)) +
  labs(x="dbRDA1 [16.8%]", y="dbRDA2 [9.8%]", tag="a")
...
dbrda.plot.b <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  geom_path(data=site.hulls[["df_hull"]], aes(x=2.45*x, y=3.2*y, color=Group), size=0.5) +
  geom_point(aes(color = Type, shape=Type), size=2, fill="white") +
  annotate("text", x=-2.5, y=1.5, hjust=1, color="#00A2BD", label="duodenum", size=3) +
  annotate("text", x=-2.25, y=-2.25, hjust=1, color="#D544D1", label="jejunum", size=3) +
  annotate("text", x=-1.5, y=-1.25, hjust=0, color="#B97700", label="ileum", size=3) +
  annotate("text", x=3, y=4.5, hjust=0, color="#00A358", label="caecum", size=3) +
  annotate("text", x=-1, y=4.5, hjust=0, color="#4C7FEE", label="asc.colon", size=3) +
  annotate("text", x=1, y=-0.35, hjust=0, color="#E14F77", label="des.colon", size=3) +
  theme_bw() +
  theme(panel.border = element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
  ...
  scale_x_continuous(breaks=c(-5,-2.5,0,2.5,5), limits=c(-5,5)) +
  scale_y_continuous(breaks=c(-7.5,-5,-2.5,0,2.5,5), limits=c(-8,6.3)) +
  labs(x="dbRDA1 [16.8%]", y="dbRDA2 [9.8%]", tag="b")
...
####  FIGURE 5 - FECAL EFFICACY PANEL ###################
# (a) Aitchison distance from feces to each sample. 
feces.plot.a <- ggplot(dist.to.feces.AIT.summary, aes(x=variable, y=value, fill=variable)) +
  ...
  theme(panel.border = element_blank(),
        ...
        plot.tag=element_text(face="bold"),
        legend.position="none") +
  labs(x="Site", y="Aitchison distance to feces", tag="a")
...
####  FIGURE S2 - MaAsLIN RESULTS ####
# Import the output data from Galaxy.
maaslin.graph.data <- read.csv("~/maaslin.results.csv")
maaslin.graph.data <- merge(maaslin.graph.data, as.data.frame(cbind(tax_table(tax_glom(pseq.culled.sub, taxrank="Genus")))), by="Genus", all=FALSE)

# Organize by Phylum, then by Family.
maaslin.graph.data <- arrange(maaslin.graph.data, Phylum, Family, Genus)
maaslin.graph.data <- mutate(maaslin.graph.data, Feature = factor(Genus, Genus))
...
levels(maaslin.graph.data$Genus)[levels(maaslin.graph.data$Genus)=="Uncl_Lachnospiraceae"] <- "Lachnospiraceae spp."
levels(maaslin.graph.data$Genus)[levels(maaslin.graph.data$Genus)=="Uncl_Rhodobacteraceae"] <- "Rhodobacteraceae spp."
levels(maaslin.graph.data$Genus)[levels(maaslin.graph.data$Genus)=="Escherichia/Shigella"] <- "Escherichia"
levels(maaslin.graph.data$Genus)[levels(maaslin.graph.data$Genus)=="Uncl_Fusobacteriaceae"] <- "Fusobacteriaceae spp."
levels(maaslin.graph.data$Genus)[levels(maaslin.graph.data$Genus)=="Clostridium_sensu_stricto"] <- "Clostridium"
maaslin.graph.subset <- subset(maaslin.graph.data, Genus %in% heatmap.data$rowname)
maaslin.graph.subset$Genus <- factor(maaslin.graph.subset$Genus, levels=levels(heatmap.data$rowname))
maaslin.graph.subset$Genus <- factor(maaslin.graph.subset$Genus)

maaslin.graph.subset$Min <- -1*maaslin.graph.subset$Min
maaslin.graph.subset$Coefficient <- -1*maaslin.graph.subset$Coefficient
maaslin.graph.subset$Max <- -1*maaslin.graph.subset$Max

colornames.maaslin <- c("#CC0000", "#CC0000", # 2x Actinobacteria
                "magenta3", "magenta3", # 3x Bacteroidetes
                "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600", "#996600",  #16x Firmicutes
                "darkgreen", "darkgreen", # 2x Fusobacteria
                "#0000FF", "#0000FF", "#0000FF", "#0000FF", "#0000FF") # 8x Proteobacteria
names(colornames.maaslin) <- levels(maaslin.graph.subset$Genus)

# Plot families against their MaAsLin coefficients.
Fig_S2 <- ggplot(maaslin.graph.subset, aes(x=Genus, y=Coefficient)) + 
  geom_point(aes(color=Phylum), size=2) +
  geom_text(aes(x=Genus,  y=0.75, label=Q.value), size=3) +
  theme_bw() +
  geom_errorbar(aes(ymin=maaslin.graph.subset$Min, ymax=maaslin.graph.subset$Max, x=Genus, color=Phylum), position=position_dodge(0.9), width=0.2) +
  geom_hline(yintercept=0) +
  labs(x="Genus", y="Coefficient") +
  scale_y_continuous(limits=c(NA,0.8), breaks=c(-0.5,-0.25,0,0.25,0.5)) +
  coord_flip() +
  scale_x_discrete(limits = rev(levels(maaslin.graph.subset$Genus))) +
  scale_color_manual(values=c("#CC0000", "#e75480", "#996600", "darkgreen", "#0000FF")) +
  theme(
    axis.text.y = element_text(size=9, color=rev(colornames.maaslin)),
...
####  FIGURE S3 - TAXON APPEARANCE AT EACH SITE ####
...
####  FIGURE S4 - UNIQUE ASVs AT EACH SITE ####
...
####  FIGURE S5 - dbRDA with OTHER DISTANCES ###########
# (a-b) Jaccard distance ####
dbrda.nofeces <- capscale(dist.JAC.nofeces ~ Individual+Segment/Type, bg.metadata.nf)
dbrda.nofeces_fort <- fortify(dbrda.nofeces, display="sites")
dbrda.nofeces_fort <- cbind(bg.metadata.nf, dbrda.nofeces_fort)
desired_order <- c("small.intestine","large.intestine")
dbrda.nofeces_fort$Segment <- factor(dbrda.nofeces_fort$Segment, levels=desired_order)
...
# Create first plot (ellipses for segment, hulls for individual).
dbrda.jac <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  
  # The individual hulls need to be scaled due to some discrepancy between the gg_ordiplot function and actual point values from dbrda.
  geom_path(data=ind.hulls[["df_hull"]], aes(x=2.5*x, y=3.75*y, linetype=Group), size=0.5, color="grey") +
  scale_linetype_manual("Individual", values=rep("solid", 20)) +
  
  geom_point(aes(color = Individual, shape=Type), size=2, fill="white") +
  
  # Manually add SI and LI labels at the segment centroids.
  geom_label(aes(x=0.63769, y=0.07049), color="black", label="LI") +
  geom_label(aes(x=-0.65968, y=-0.07292), color="black", label="SI") +
  new_scale("linetype") +
  ...
  scale_color_manual(values=dbrda.indv.pal(10), guide=FALSE) +
  scale_shape_manual(values=c(22,21,24,15,16,17)) +
  labs(x="dbRDA1 [15.9%]", y="dbRDA2 [7.06%]", tag="a")
...
# Create plot.
dbrda.jac.hull <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  geom_path(data=site.hulls[["df_hull"]], aes(x=2.5*x, y=3.75*y, color=Group), size=0.5) +
  geom_point(aes(color = Type, shape=Type), size=2, fill="white") +
  annotate("text", x=-1, y=1.1, hjust=1, color="#00A2BD", label="duodenum", size=3) +
  annotate("text", x=-0.9, y=-1, hjust=1, color="#D544D1", label="jejunum", size=3) +
  annotate("text", x=-0.5, y=0.9, hjust=0, color="#B97700", label="ileum", size=3) +
  annotate("text", x=0.7, y=1.2, hjust=0, color="#00A358", label="caecum", size=3) +
  annotate("text", x=-0.05, y=1.7, hjust=0, color="#4C7FEE", label="asc.colon", size=3) +
  annotate("text", x=0.25, y=-1.5, hjust=0, color="#E14F77", label="des.colon", size=3) +
  theme_bw() +
  theme(panel.border = element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
  ...
  #scale_x_continuous(breaks=c(-5,-2.5,0,2.5,5), limits=c(-5,5)) +
  #scale_y_continuous(breaks=c(-7.5,-5,-2.5,0,2.5,5), limits=c(-8,6.3)) +
  labs(x="dbRDA1 [15.9%]", y="dbRDA2 [7.06%]", tag="b")


# (c-d) Unweighted UniFrac distance ####
dbrda.nofeces <- capscale(dist.uwUF.nofeces ~ Individual+Segment/Type, bg.metadata.nf)
dbrda.nofeces_fort <- fortify(dbrda.nofeces, display="sites")
dbrda.nofeces_fort <- cbind(bg.metadata.nf, dbrda.nofeces_fort)
desired_order <- c("small.intestine","large.intestine")
dbrda.nofeces_fort$Segment <- factor(dbrda.nofeces_fort$Segment, levels=desired_order)
...
# Create first plot (ellipses for segment, hulls for individual).
dbrda.uwUF <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  
  # The individual hulls need to be scaled due to some discrepancy between the gg_ordiplot function and actual point values from dbrda.
  geom_path(data=ind.hulls[["df_hull"]], aes(x=2.2*x, y=4.05*y, linetype=Group), size=0.5, color="grey") +
  scale_linetype_manual("Individual", values=rep("solid", 20)) +
  
  geom_point(aes(color = Individual, shape=Type), size=2, fill="white") +
  
  # Manually add SI and LI labels at the segment centroids.
  geom_label(aes(x=0.589022, y=-0.034793), color="black", label="LI") +
  geom_label(aes(x=-0.609333, y=0.035992), color="black", label="SI") +
  new_scale("linetype") +
  ...
  scale_color_manual(values=dbrda.indv.pal(10), guide=FALSE) +
  scale_shape_manual(values=c(22,21,24,15,16,17)) +
  labs(x="dbRDA1 [20.8%]", y="dbRDA2 [6.05%]", tag="c")
...
# Create plot.
dbrda.uwUF.hull <- ggplot(dbrda.nofeces_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  geom_path(data=site.hulls[["df_hull"]], aes(x=2.2*x, y=4.05*y, color=Group), size=0.5) +
  geom_point(aes(color = Type, shape=Type), size=2, fill="white") +
  annotate("text", x=-1, y=0.75, hjust=0, color="#00A2BD", label="duodenum", size=3) +
  annotate("text", x=-0.9, y=-1.4, hjust=1, color="#D544D1", label="jejunum", size=3) +
  annotate("text", x=-0.1, y=0.9, hjust=0, color="#B97700", label="ileum", size=3) +
  annotate("text", x=0.7, y=1.4, hjust=0, color="#00A358", label="caecum", size=3) +
  annotate("text", x=0.25, y=1.7, hjust=0, color="#4C7FEE", label="asc.colon", size=3) +
  annotate("text", x=0.25, y=-1.5, hjust=0, color="#E14F77", label="des.colon", size=3) +
  theme_bw() +
  theme(panel.border = element_blank(),
        panel.grid.major = element_blank(), 
  ...
  #scale_x_continuous(breaks=c(-5,-2.5,0,2.5,5), limits=c(-5,5)) +
  #scale_y_continuous(breaks=c(-7.5,-5,-2.5,0,2.5,5), limits=c(-8,6.3)) +
  labs(x="dbRDA1 [20.8%]", y="dbRDA2 [6.05%]", tag="d")

# (e-f) Weighted UniFrac distance. ####
dbrda.wUF <- capscale(dist.wUF.nofeces ~ Individual+Segment/Type, bg.metadata.nf)
...
dbrda.wUF.plot.a <- ggplot(dbrda.wUF_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  geom_path(data=ind.hulls[["df_hull"]], aes(x=1.85*x, y=3.4*y, linetype=Group), size=0.5, color="grey") +
  scale_linetype_manual("Individual", values=rep("solid", 20)) +
  geom_point(aes(color = Individual, shape=Type), size=2, fill="white") +
  geom_label(aes(x=0.47564, y=0.05703), color="black", label="LI") +
  geom_label(aes(x=-0.49204, y=-0.05899), color="black", label="SI") +
  new_scale("linetype") +
  ...
  scale_color_manual(values=dbrda.indv.pal(10), guide=FALSE) +
  scale_shape_manual(values=c(22,21,24,15,16,17)) +
  labs(x="dbRDA1 [26.0%]", y="dbRDA2 [8.0%]", tag="e")
...
dbrda.wUF.plot.b <- ggplot(dbrda.wUF_fort, aes(x=CAP1, y=CAP2)) +
  geom_vline(xintercept = 0, color="grey") +
  geom_hline(yintercept = 0, color="grey") +
  geom_path(data=site.hulls[["df_hull"]], aes(x=1.85*x, y=3.4*y, color=Group), size=0.5) +
  geom_point(aes(color = Type, shape=Type), size=2, fill="white", alpha=1) +
  annotate("text", x=-0.8, y=-0.8, hjust=0, color="#00A2BD", label="duodenum", size=3) +
  annotate("text", x=-0.88, y=1, hjust=0, color="#D544D1", label="jejunum", size=3) +
  annotate("text", x=-0.55, y=1.25, hjust=0, color="#B97700", label="ileum", size=3) +
  ...
  scale_color_manual(values=dbrda.indv.pal(6), guide=FALSE) +
  scale_shape_manual(values=c(22,21,24,15,16,17)) +
  labs(x="dbRDA1 [26.0%]", y="dbRDA2 [8.0%]", tag="f")

grid.arrange(dbrda.wUF.plot.a, dbrda.wUF.plot.b, dbrda.legend,
             layout_matrix=rbind(c(1,2),c(3)), heights=c(0.4,0.1))

Fig_S5 <- arrangeGrob(dbrda.jac, dbrda.jac.hull, dbrda.uwUF, dbrda.uwUF.hull, dbrda.wUF.plot.a, dbrda.wUF.plot.b, dbrda.legend,
                           layout_matrix=rbind(c(1,2),c(3,4),c(5,6),c(7)), heights=c(0.8,0.8,0.8,0.2))
ggsave("~/Fig_S5.jpg", Fig_S5, width=6.5, height=7, units="in", dpi=300)

rm(dbrda.jac, dbrda.jac.hull, dbrda.uwUF, dbrda.uwUF.hull, dbrda.wUF.plot.a, dbrda.wUF.plot.b, dbrda.legend, site.hulls, ind.hulls)

####  FIGURE S6 - RANDOM FOREST RESULTS ####
...
rf.graph.data.seg$Max <- factor(rf.graph.data.seg$Max, levels=c("small.intestine","large.intestine"))
levels(rf.graph.data.seg$Max)[levels(rf.graph.data.seg$Max)=="small.intestine"] <- "small int."
levels(rf.graph.data.seg$Max)[levels(rf.graph.data.seg$Max)=="large.intestine"] <- "large int."
...
####  FIGURE S7 - RICHNESS AND DIVERSITY, FACETED BY INDIVIDUAL ####
...
####  FIGURE S8 - TAXONOMIC BIAS in FECES-UNDETECTED TAXA ####
...
####  FIGURE S9 - ASV ABUNDANCES SCATTER PLOT MATRIX ####
